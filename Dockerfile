...
WORKDIR /app
RUN apt-get update && apt-get install -y binutils procps && apt-get clean
...
COPY . .
EXPOSE 8080
CMD ["gunicorn", "--log-file=-", "--capture-output", "--log-level=debug", "--error-log=-", "--access-logfile=-", "--bind=0.0.0.0:8080", "app:create_app()"]
